...
public class RestSimulatorUtilsTest {
	...
	private void verify(final Map<String, String> expectedParameters, final Map<String, String> resultParameters) {
		for (final Map.Entry<String, String> entry : resultParameters.entrySet()) {
			...
			Assert.assertEquals(String.format("Verify parameter map  EXPECTED VALUE %s RECEIVED VALUE %s USING KEY %s ", expectedParameters.get(key), value, key), expectedParameters.get(key), value);
		}
	}
	...
	public void buildQueryStringFromInputStream() throws SimulatorException {
		...
		Assert.assertEquals(String.format("Verify QueryString  EXPECTED VALUE \'%s\' RECEIVED VALUE \'%s\'.", expectedQueryString, resultQueryString), expectedQueryString, resultQueryString);
	}
